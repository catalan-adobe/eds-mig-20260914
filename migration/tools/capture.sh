#!/usr/bin/env bash
# usage: tools/capture.sh <session> <url> <out-prefix> <w> <h> <css-file> <prep-file> [fullpage-css-file]
...
node -e '
const fs = require("fs");
const [tpl, url, out, w, h, cssf, prepf, fullf] = process.argv.slice(1);
const esc = (s) => s.replace(/\\/g, "\\\\").replace(/`/g, "\\`");
let s = fs.readFileSync(tpl, "utf8");
s = s.replace("__URL__", url).replace(/__OUT__/g, out).replace("__W__", w).replace("__H__", h)
...
fs.writeFileSync(process.argv[9], s);
' "$dir/capture.tpl.js" "$2" "$3" "$4" "$5" "$6" "$7" "${8:-}" "$tmp"
"$dir/pw.sh" "$1" "$tmp"
